...
class DomainAnalyzer:
    ...
    def getDomainStats(self,getCenter=True, getArea=True, getVol=True, getIQ=True, plotMesh=False):
        ...
        #tmp = np.ravel(self.__borderID)
        #tmp = np.resize(tmp,(1,len(tmp)))
        #PolyFTS_to_VTK.writeVTK("borderID.vtk", self.__Nx, True, self.__M, AllCoords,tmp)
        ...
        for idomain in range(1,self.__ndomains+1):
            icol = 0 
            stats[idomain-1,icol] = idomain; icol += 1

            # calc center of domain
            if getCenter:
                com = self.calcDomainCOM(idomain,units='coord')
                for i in range(self.__ndim):
                    stats[idomain-1,icol] = com[i]; icol += 1
            ...
            if getArea or getVol or getIQ:
                ...
                if getArea or getIQ:
                    ...
                    if getArea:
                        stats[idomain-1,icol] = surface_area; icol += 1
                if getVol or getIQ:
                    volume = self.mesh_volume(verts,faces)
                    if getVol:
                        stats[idomain-1,icol] = volume; icol += 1
                if getIQ:
                    IQ = self.calcIQ(surface_area, volume)
                    stats[idomain-1,icol] = IQ; icol += 1
            
            if plotMesh: 
                self.plotMesh(verts,faces, "mesh.{}.png".format(idomain))
